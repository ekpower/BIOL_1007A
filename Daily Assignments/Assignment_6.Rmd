...
```{r}
irisTab <- summarize(iris4, Mean.Sepal.Length = mean(Sepal.Length), Mean.Sepal.Width = mean(Sepal.Width), Sample.Size = n())
irisTab
```
...
iris5 <- iris4 %>%
  group_by(Species) %>%
  summarize(Mean.Sepal.Length = mean(Sepal.Length), Mean.Sepal.Width = mean(Sepal.Width), Sample.Size = n())
iris5
```
