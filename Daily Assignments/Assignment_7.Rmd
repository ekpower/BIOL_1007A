...
```{r}
species_petalLength <- filter(iris)



homeworld_counts <- starwars %>% 
  count(homeworld)
glimpse(homeworld_counts)

homeworld_names <- filter(homeworld_counts, n>1)
homeworld_names

filtered_starwars2 <- filter(starwars, homeworld %in% c("Alderaan", "Corellia", "Coruscant", "Kamino", "Kashyyyk", "Mirial", "Naboo", "Ryloth", "Tatooine"))
head(filtered_starwars2)
anyNA(filtered_starwars2[,10])

clean_starwars2 <- filtered_starwars2[complete.cases(filtered_starwars2[,8]),]  # filter NAs from 'sex' column

p2 <- ggplot(clean_starwars2, aes(x="homeworld", y=sex, fill=sex)) +
